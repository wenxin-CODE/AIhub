...
# 读取技能文件内容
with open("D:\Python\AI Agent\Skills\skill.md", "r", encoding="utf-8") as f:
    skill_content = f.read()

# 创建系统提示词
system_prompt = f"""你是一名资深美股分析师，专注于美股七大科技巨头的投资分析。
...
def run_analysis():
    ...
    try:
        # 执行 Agent
        result = agent.invoke({
            "messages": [("human", f"请开始执行美股七大科技巨头的每日投资分析任务。当前日期：{current_date}")]
        })
        ...
        # 保存报告到文件
        report_filename = f"investment_report_{current_date}.md"
